CREATE OR REPLACE TRIGGER MFC_LDG_ACS_FND_V_IOIT
INSTEAD OF INSERT ON MFC_LDG_ACS_FND_V
FOR EACH ROW 
DECLARE
...
BEGIN  
  ...
  THEN -- check for finding in predecessor recon run 
    BEGIN 
      ...
      ( NVL(:new.LDG_ACC_NAME, 'NULL'), NVL(TO_CHAR(:new.PRICE_PER_UNIT), 'NULL'),
        ...
        NVL(TO_CHAR(:new.D2K_CUM_CNT), 'NULL'), NVL(TO_CHAR(:new.D2K_CUM_ACC_AMOUNT), 'NULL'), NVL(TO_CHAR(:new.D2K_CUM_ACC_AMOUNT_CHF), 'NULL'),
        'MFCLFA'
      ) IN 
      ( SELECT 
          ...
          NVL(TO_CHAR(D2K_CUM_CNT), 'NULL'), NVL(TO_CHAR(D2K_CUM_ACC_AMOUNT), 'NULL'), NVL(TO_CHAR(D2K_CUM_ACC_AMOUNT_CHF), 'NULL'),
          LDG_FND_TYPE
        FROM MFC_LDG_FND 
  ...
  ( MFC_RCN_FND_FND_ID_SEQ.NEXTVAL, 
    ...
    NULL,
    :new.D2K_CUM_CNT,
    :new.D2K_CUM_ACC_AMOUNT,
    :new.D2K_CUM_ACC_AMOUNT_CHF,
    'MFCLFA',
